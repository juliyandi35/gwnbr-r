# Input DBD
data <- read.csv("D:/TA/TA/Pendidikan1.csv", sep = ";")
data
View(data)
summary(data[2:9])
...
modelnegbin=glm(Putus~Miskin+IPM+RLS+HLS+APS+TPT+PDRB,family=
                  negative.binomial(1.753), data=data) 
summary(modelnegbin) 
...
write.table(jarak, file = "D:/TA/TA/jarak.csv", sep = ";")
jarak
...
write.table(pembobotB,file="D:/TA/TA/pembobotB1.csv",sep=";")
...
mod=gwnbr1(x,y,pembobotB, 1.753) 
mod$Z_hitung
mod$koefisien 
write.table(mod$Z_hitung,file="D:/TA/TA/z_hitungF1.csv",sep=";") 
write.table(mod$koefisien,file="D:/TA/TA/koefisienF1.csv",sep=";")
...
Lwgw <- sum(slr-log(factorial(datay))+datay*log(muwgw)-
              (datay+1/tetagw)*log(tetagw*muwgw+1))
Logw <- sum(slr-log(factorial(datay))+datay*log(muogw)-
               (datay+1/tetagw)*log(tetagw*muogw+1))
DGW <- 2*(Logw-Lwgw)
DGW
